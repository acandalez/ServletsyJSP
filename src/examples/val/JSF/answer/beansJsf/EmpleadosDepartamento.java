package examples.val.JSF.answer.beansJsf;
...
import examples.val.JSF.answer.services.EmpleadoServices;

//las clases anotadas con esta ruta (annotation), son anotaciones gestionadas por las propias librerias del Contenedor Tomcat
...
public class EmpleadosDepartamento implements Serializable {
	...
	private int posicionDepartamento(int id_departamento_buscado,
			List<Departments> lista_departamentos) {
		int n = -1;
	...
	public void init() {
		...
		/*
		 * Departamentos dptos =
		 * (Departamentos)FacesContext.getCurrentInstance()
		 * .getExternalContext().getSessionMap().get("departamentos");
		 * List<Departments> lista_departamentos =
		 * dptos.getListaDepartamentos(); int pos_depto_seleccionado =
		 * posicionDepartamento(id_departamento, lista_departamentos);
		 * conjuntoEmpleados =
		 * lista_departamentos.get(pos_depto_seleccionado).getEmployeeses();
